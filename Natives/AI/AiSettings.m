...
+ (NSString *)defaultSystemPrompt {
    ...
    "5. 安装到哪个游戏目录？用户不确定时：先用 list_instances 检测当前游戏目录已安装的游戏版本是否符合要求（版本与加载器匹配），符合则直接安装到当前目录；不符合则再次询问用户（可建议新建目录，见第一节第 3 条）。\n"
    ...
    "2. 安装游戏本体和加载器前，先查看版本列表（list_game_versions，默认源 BMCLAPI，失败自动切官方）。\n"
